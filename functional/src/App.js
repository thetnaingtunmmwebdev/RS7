...
const Item = ({name, price, remove, _id}) => {
  return (
    <li>{name},${price} <a href="#/" onClick={remove(_id)}>&times;</a></li>
  )
}
...
const App = props => {
  ...
  const [fruits, setFruits] = useState([
    ...
    {_id: 2, name: 'Banana', price: 5.55},
    {_id: 3, name: 'Coconut', price: 5.55},
    {_id: 4, name: 'Watermelon', price: 5.55},
  ]);
  ...
  return (
    <div>
      ...
      <ul>
        {fruits.map(fruit => {
          return (
            <Item key={fruit._id} _id={fruit._id} name={fruit.name} price={fruit.price} remove={remove} />
          )
        })}
      </ul>
